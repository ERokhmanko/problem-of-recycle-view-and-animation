...
class PostsAdapter(
    ...
    override fun onBindViewHolder(holder: PostViewHolder, position: Int) {
        (holder.binding.root.tag as? Animator)?.cancel() // проверяем есть ли уже какой-то аниматор и чистим
...
class PostViewHolder(
    ...
    fun bind(payload: Payload) {
        payload.liked?.also { liked ->
            ...
            if (liked) {
                // привязываем аниматор к полю
                val animatorLiked = ObjectAnimator.ofPropertyValuesHolder(
                    binding.like,
                ...
                // присваиваем тэгу вью аниматор
                itemView.tag = animatorLiked

            } else {
                // привязываем аниматор к полю
                val animatorNotLiked = ObjectAnimator.ofFloat(
                    binding.like,
                    View.ROTATION,
                    0F, 360F
                ).apply { start() }
                //  присваиваем тэгу вью аниматор
                itemView.tag = animatorNotLiked
            }
